...
export const FIXED_ACQUISITION_ASSUMPTIONS = [
  ...
  // 목표가가 전용 85㎡ 이하 실거래에서 나온 값이라 84로 고정했다(85㎡ 이하는
  // 국민주택규모로 확정돼 농어촌특별세가 붙지 않는다). 실제 면적이
  // 85~100㎡이면 엔진은 국민주택규모 해당 여부를 확정하지 못해(None) 세액이
  // 정해지지 않는데, 84로 고정하면 이 미확정 상태가 "붙지 않음"(True)으로
  // 확정돼버린다 — 취득세를 과소 계산하는 방향이라 고급주택 가정과 같은
  // 이유로 밝힌다.
  "전용면적은 84㎡(국민주택규모 이하)로 계산했습니다. 실제 면적이 85㎡를 넘으면 농어촌특별세가 붙어 실제 취득세는 이 계산보다 커질 수 있습니다.",
] as const;

...
```
/                        랜딩 — [금융 라이프 컨설팅 받기] → step 1로 직행
/personas                페르소나 선택 — 20명 카드 그리드 (보조 경로)
                           필터: 기본형 7 / 여유형 6 / 취약형 7
/input?persona={id}      3-step 입력 위저드
   step 1  정보 입력     사용자가 직접 입력하는 값 전부 (프리필·편집 가능)
                           페르소나 선택기(부가) · 기본정보 · 목표설정 · 저축계획
   step 2  마이데이터 연동 [마이데이터 불러오기] → 탭 4종
                           계좌 · 예적금 · 대출 · 거래내역
   step 3  입력 확인     입력값 + 프로필 확인 정보 + 마이데이터 요약 → [결과 보기]
...
단계 구분 기준은 **입력 주체**다. 사용자가 타이핑하는 값은 step 1에 모으고, 자동으로
채워지는 값은 step 2에 모으고, 둘을 합쳐 보여주는 검토 화면을 step 3에 둔다. 검증도
step 1에서만 돌면 된다.

페르소나 선택기는 프로토타입 전용 부가 기능이다. `persona-picker.tsx` 한 파일과
`step-input.tsx`의 호출 한 줄로 격리해서, 실사용 전환 시 그 두 곳만 지우면 되게 한다.

그래서 랜딩의 시작 버튼은 `/personas`를 거치지 않고 step 1로 바로 간다. 실사용이라면
빈 폼으로 시작하겠지만 프로토타입은 프리필된 값이 있어야 결과까지 이어지므로, 픽스처
목록의 첫 페르소나로 진입한 뒤 step 1의 선택기로 바꾼다. `/personas`는 남겨 둔다 —
목록에 없는 `?persona=` 값이 들어왔을 때의 리다이렉트 대상이고, 20명을 한눈에 비교하는
보조 화면으로도 쓴다.

**step 1이 받는 값** (설계안 6.1 중 예적금 엔진이 실제로 쓰는 것만):
나이 · 가구원수 · 월 소득 · 월 평균 지출 · 보유 자산(있을 때) ·
목표 금액 · 목표 시점 · 위험 성향 · 월 저축 예산 · 일시 예치금 · 비상 예비금.

`target_monthly_rent`(목표 월세)와 `target_management_fee`(목표 관리비)는 **입력에서
제외한다.** 이 서비스는 매매 계약을 위한 자금 포트폴리오를 다루므로 임대료 항목이
필요 없고, 예적금 엔진 입력에도 들어가지 않는다.

목표 시점 입력 형식은 `YYYY-MM`이다. 픽스처·백엔드 계약은 `YYYYMM`을 쓰므로 경계에서
변환한다(`formatYmInput` / `parseYmInput`).

> **목표 금액 라벨에 관한 주의.** 화면에서는 "목표 금액"으로 부른다. 20명의
> `target_price`는 원본이 월세 보증금 시나리오(`target_housing_type: "monthly_rent"`,
> 300만~8,000만원)로 생성된 값이므로 이를 "매매가"로 표기하면 화면이 거짓을 말하게
> 된다. 매매 시나리오 페르소나는 a~d(4.5억~11억)이고 현금흐름 엔진이 비어 있어 1차
> 범위에서 제외했다(§8). step 1의 목표 설정 그룹에 이 사실을 한 줄로 명시한다.
...
- [ ] step 3에서 입력값·프로필 정보·마이데이터 연동 결과를 한 화면에서 확인하고 [결과 보기]로 넘어간다
